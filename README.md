...
为 [my-neuro](https://github.com/A-night-owl-Rabbit) 框架开发的心情主动对话插件。AI 的心情如潮汐般涨落——根据当前心情自动调整主动对话频率，并通过多维度评估实时更新心情状态。

## 前置依赖

> **必须配套安装 [AI 日志插件 (ai-log)](https://github.com/A-night-owl-Rabbit/my-neuro-plugin-ai-log)**

潮汐之音的 **初始心情评估** 功能依赖 ai-log 插件生成的每日 AI 日志。两个插件配合使用，可以实现：

- **跨会话心情记忆** — AI 重启后不会"失忆"，而是根据上一天日志中的互动质量和情绪倾向自动恢复心情。昨天聊得开心，今天一启动就元气满满；昨天闹了别扭，今天就会有点闷闷不乐
- **心情有据可依** — 心情不再是一个凭空的数字，而是基于真实对话记录，由独立 LLM 从"整体基调、结尾情绪、互动关系"三个维度加权评估得出
- **日积月累的情感曲线** — 配合 ai-log 的月度总结功能，AI 不仅记住每天的心情，还能形成长期的情感趋势感知，对话风格会随相处时间自然演变
- **人格连贯性** — ai-log 将日志写入核心记忆，潮汐之音从日志评估情绪状态，两者共同构建了一个"有昨天、有今天"的 AI 人格

如果不安装 ai-log 插件，潮汐之音仍然可以正常运行——实时情感分析、回复速度感知、心情回归等功能不受影响。但每次重启后 AI 会从默认心情值开始，无法继承上一天的情绪状态。
...
- **[AI 日志插件 (ai-log)](https://github.com/A-night-owl-Rabbit/my-neuro-plugin-ai-log)** — 生成每日 AI 日志供初始心情评估（强烈推荐）
